 ...
 editor: function(options) {
   ...
   _renderLexonomyNotes: function() {
      ...
      if (lexonomy_notes.length) {
         for (lexonomy_note of lexonomy_notes) {
            let lexonomy_noteRow = $(`<div class="lexonomy_noteRow displayFlex alignCenter pl-8"></div>`).appendTo(this.contentNode)
            $(`<div>${lexonomy_note.value}</div>`).appendTo(lexonomy_noteRow)
         }
      } else {
         $(`<div class="grey-text ml-8">no notes</div>`).appendTo(this.contentNode)
      }
      $(`<button class="btn btn-secondary ml-8"><i class="material-icons left">add</i>add note</button>`).appendTo(this.contentNode)
   ...
   _addLexonomyNote() {
      if(lexonomy_note_value = prompt("Enter the text of your note:")) {
      ...
      this.entry.children = this.entry.children.filter(child => child.name != "lexonomy_flag")
      this.entry.children.push({
         name: "lexonomy_flag",
         value: "pending",
         parent: this.entry,
      ...
      this._validate()
      this.options.onChange()
      if(window.nvhStore.data.isValid == true) window.nvhStore.saveEntry()
   },
