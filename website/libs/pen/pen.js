...
(function(root, doc) {
  ...
  var commandsReg = {
    ...
    inline: /^(?:bold|italic|underline|insertorderedlist|insertunorderedlist|indent|outdent|undo|redo)$/,
  ...
  function initToolbar(ctx) {
    ...
    if (!ctx._toolbar) {
      ...
      utils.forEach(toolList, function (name) {
        var tooltip = ctx.config.tooltips[name] || '';
        var classes = `pen-icon icon-${name} ${tooltip ? 'tooltipped' : ''}`;
        var icon = ctx.config.icons[name] || '';
        icons += `<i class="material-icons ${classes}" data-action="${name}" data-tooltip="${tooltip}">${icon}</i>`;
      }, true);
    }
    ...
    if (icons) {
      ...
      (ctx.config.toolbarNode || doc.body).appendChild(ctx._menu);
    }
    if (toolList.indexOf('createlink') >= 0 || toolList.indexOf('insertimage') >= 0){
      ctx._inputBar = doc.createElement('input');
      ctx._inputBar.setAttribute('class', 'pen-input');
      ctx._inputBar.setAttribute('placeholder', 'https://');
      (ctx.config.toolbarNode || doc.body).appendChild(ctx._inputBar);
    }
  ...
  function initEvents(ctx) {
    ...
    // prevent google translate icon to pop up on undo/redo, selecting text,...
    addListener(ctx, toolbar, 'mouseup', function(e) {e.stopPropagation()})
    addListener(ctx, editor, 'mouseup', function(e) {e.stopPropagation()})
  }
  ...
  Pen.prototype.execCommand = function(name, value) {
    ...
    } else if (this.config.customCommands[name]) {
      this.config.customCommands[name](value)
    } else {
