window.nvhPlugins.voiceRss = {
   ...
   isActive: function(element){
      ...
      return element.name == config.titling.headword
            && config.gapi.voicekey
            && config.gapi.voicelang != ''
            && !!window.nvhStore.findElement(e => e.name == window.store.data.config.titling.headword).value
   },

   onPluginButtonClick: function(){
      let gapi = window.store.data.config.gapi
      let headword = window.nvhStore.findElement(e => e.name == window.store.data.config.titling.headword).value
